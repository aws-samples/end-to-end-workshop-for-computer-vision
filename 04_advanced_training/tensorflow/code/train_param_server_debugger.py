...
## Perform validation
def perform_validation(model, val_dataset, num_valid_images, batch_size,
                       val_acc_metric):
...
if __name__=='__main__':
    ...
    train_dataset, valid_dataset = create_data_generators(args)
    
    num_classes = len(glob.glob(f'{args.train}/*'))
    print(f'Number of training classes: {num_classes} =================')
    
    num_train_images = len(train_dataset.filepaths)
    num_valid_images   = len(valid_dataset.filepaths)
